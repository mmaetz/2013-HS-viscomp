...
\item[\lp{Fisher linear discriminant analysis}] Eigenimage method maximizes ``scatter'' within the linear subspace over the entire image set - regardless of classification task
	...
	\item[\lp{Entropy Coding (Huffman code)}]
%		\newcolumntype{d}{D{.}{.}{-1}}
		\begin{tabular}[]{c@{\hskip -1ex} S@{\hskip -1ex} r@{\hskip -1ex} S}
			\multicolumn{1}{c@{\hskip -1ex}}{symbol}&\multicolumn{1}{c@{\hskip -1ex}}{prob.}&\multicolumn{1}{c@{\hskip -1ex}}{code}&\multicolumn{1}{c}{binary frac.}\\
			\toprule
			$Z$&0.5&1&0.1\\
			$Y$&0.25&01&0.01\\
			$X$&0.125&001&0.001\\
